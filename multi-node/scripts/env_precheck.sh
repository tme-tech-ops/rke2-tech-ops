...
cleanup() {
    rc=$?
    ctx logger info "env_precheck.sh exiting with code ${rc}"
    exit $rc
}
trap cleanup EXIT

# CP1 install-only precheck: download installer and assemble install command
...
# Assemble install command (CP1 always installs, never joins)
INSTALL_CMD="install"
